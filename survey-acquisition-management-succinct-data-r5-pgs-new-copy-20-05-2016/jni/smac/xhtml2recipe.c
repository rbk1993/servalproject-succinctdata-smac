...
//	    free(curr_r->xhtml2recipe[curr_r->xhtml2recipeLen]);
	  ...
	  if (!strcasecmp(node_name,"instanceID")) {
	    ...
	    printf("xhtml2recipeLen of current recipe = %d\n",curr_r->xhtml2recipeLen);
		  ...
		  char *templatetext,int *templateLen)
{
  ...
  {
	  // Build recipe output
	    int recipeMaxLen=recipenode->recipeLen;
	    recipenode->recipeLen=0;

	    // Start with implied fields
	    strcpy(recipenode->recipetext,implied_meta_fields);
	    recipenode->recipeLen=strlen(recipenode->recipetext);

	    // Now add explicit fields
	    for(i=0;i<recipenode->xhtml2recipeLen;i++){
	      if (appendto(recipenode->recipetext,&(recipenode->recipeLen),recipeMaxLen,recipenode->xhtml2recipe[i])) {
	        fprintf(stderr,"ERROR: %s:%d: %s() recipe text overflow.\n",
	  	      __FILE__,__LINE__,__FUNCTION__);
	        return -1;
	      }
	      if (appendto(recipenode->recipetext,&(recipenode->recipeLen),recipeMaxLen,"\n")) {
	        fprintf(stderr,"ERROR: %s:%d: %s() recipe text overflow.\n",
	  	      __FILE__,__LINE__,__FUNCTION__);
	        return -1;
	      }
	    ...
	    for(i=0;i<recipenode->xhtmlSelectsLen;i++){
	      if (appendto(recipenode->recipetext,&(recipenode->recipeLen),recipeMaxLen,recipenode->selects[i])) {
	        fprintf(stderr,"ERROR: %s:%d: %s() recipe text overflow.\n",
	  	      __FILE__,__LINE__,__FUNCTION__);
	        return -1;
	      }
	  ...
	  fprintf(stderr,"xhtmlFormName=%s, xhtmlFormVersion=%s\n",
	  	  recipenode->formname,recipenode->formversion);
