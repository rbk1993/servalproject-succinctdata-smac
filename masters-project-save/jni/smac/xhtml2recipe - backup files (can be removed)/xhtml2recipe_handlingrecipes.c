...
char     *xhtmlFormName[MAXRECIPES], *xhtmlFormVersion[MAXRECIPES];

int formNameCount = 0;
int formVersionCount = 0;
 ...
 ...
  ...
  //We reached the start of the data in the instance, so start collecting fields
	...
	if (!strcasecmp("id",attr[i])) {
	  xhtmlFormName[formNameCount++]  = strdup(attr[i+1]);
	  printf("form name %i = %s \n",formNameCount-1,xhtmlFormName[formNameCount-1]);
	}
	if (!strcasecmp("dd:formid",attr[i])) {
	  xhtmlFormVersion[formVersionCount++] = strdup(attr[i+1]);
	  printf("form version %i = %s \n",formVersionCount-1,xhtmlFormVersion[formVersionCount-1]);
	}
      }
    }
...
int xhtml_recipe_create(char *input)
{
  ...
  char recipetext[MAXRECIPES][65536];
  int recipeLen[MAXRECIPES];
  int m=0;
  for(m=0;m<MAXRECIPES;m++){
	  recipeLen[m] = 65536;
  }
  ...
  char formname[MAXRECIPES][1024];//[1024];
  char formversion[MAXRECIPES][1024];//[1024];

  printf("000000000001 \n");
  int r = xhtmlToRecipe(xmltext,size,(char**)formname,(char**)formversion,
			(char**)recipetext,(int**)&recipeLen,
  ...
  int i;
  //Create output for RECIPE. We Can have 1 to MAXRECIPES recipes (form recipes and optional subforms recipes)
  for(i=0;i<formVersionCount;i++) {
		snprintf(filename[i], 512, "%s.recipe", (char *)formversion[i]);
		fprintf(stderr, "Writing recipe to '%s'\n", (char *)filename[i]);
		f = fopen(filename[i], "w");
		fprintf(f, "%s", (char*)recipetext[i]);
		fclose(f);
  }

  //Create output for TEMPLATE. We always have only 1 template
  		snprintf(filename[0], 512, "%s.template", (char *)formversion[0]);
		  ...
		  char *templatetext,int *templateLen)
{
  ...
  // Build recipe output
  	  int recipeMaxLen;
  for(i=0;i<formVersionCount;i++) {
	  recipeMaxLen=recipeLen[i];
	  recipeLen[i]=0;
  }
  // Start with implied fields

  for(i=0;i<formVersionCount;i++) {
	  strcpy((char*)recipetext[i],implied_meta_fields);
	  recipeLen[i]=strlen((char*)recipetext[i]);
  }

  printf("000000000008 \n");
  // Now add explicit fields
  for(i=0;i<xhtml2recipeLen;i++){
    if (appendto((char*)recipetext,recipeLen,recipeMaxLen,xhtml2recipe[i])) {
      fprintf(stderr,"ERROR: %s:%d: %s() recipe text overflow.\n",
	      __FILE__,__LINE__,__FUNCTION__);      
      return -1;
    }
    if (appendto((char*)recipetext,recipeLen[i],recipeMaxLen,"\n")) {
      fprintf(stderr,"ERROR: %s:%d: %s() recipe text overflow.\n",
	      __FILE__,__LINE__,__FUNCTION__);      
      return -1;
    }
  ...
  printf("00000000009 \n");
  for(i=0;i<xhtmlSelectsLen;i++){
    if (appendto((char*)recipetext,recipeLen[i],recipeMaxLen,selects[i])) {
      fprintf(stderr,"ERROR: %s:%d: %s() recipe text overflow.\n",
	      __FILE__,__LINE__,__FUNCTION__);      
      return -1;
    }
  ...
  *templateLen=0;
  ...
  int k;
  printf("000000000011 \n");
  for(k=0;k<formNameCount;k++) {
	  snprintf((char *)formname[k],1024,"%s",xhtmlFormName[k]);
	  printf("formname KKK = %s \n",(char *)formname[k]);
	#ifdef ANDROID
	  LOGI("xhtmlToRecipe(): formname='%s'",formname[k]);
	#endif
  }
  printf("000000000012 \n");
  for(k=0;k<formVersionCount;k++) {
	  snprintf((char *)formversion[k],1024,"%s",xhtmlFormVersion[k]);
	#ifdef ANDROID
	  LOGI("xhtmlToRecipe(): formversion='%s'",formversion[k]);
	#endif
  }
  ...
  for(k=0;k<formVersionCount;k++){
	  fprintf(stderr,"xhtmlFormName=%s, xhtmlFormVersion=%s\n",
			  xhtmlFormName[k],xhtmlFormVersion[k]);
  }
